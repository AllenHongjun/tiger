...
namespace Tiger.Module.Notifications
{
    ...
    public class NotificationStore : INotificationStore
    {
        ...
        public async virtual Task ChangeUserNotificationReadStatusAsync(
            Guid? tenantId,
            Guid userId,
            long notificationId,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            using (_currentTenant.Change(tenantId))
            {
                ...
                {
                    notifications.ForEach(notification => notification.ChangeReadStatus(readState));
            ...
            CancellationToken cancellationToken = default)
        {
            using (var unitOfWork = _unitOfWorkManager.Begin())
            {
                ...
                foreach (var notification in notitications)
                {
            ...
            CancellationToken cancellationToken = default)
        {
            using (_currentTenant.Change(tenantId))
            {
                ...
                var userSubscriptions = await _userSubscribeRepository
                    .GetUserSubscriptionsAsync(notificationName, userIds, cancellationToken);
            ...
            using (_currentTenant.Change(notification.TenantId))
            {
                ...
                foreach (var userId in userIds)
                {
                    ...
                    if (!await _userNotificationRepository.AnyAsync(userId, notification.GetId(), cancellationToken))
                    {
                        ...
                        await _userNotificationRepository.InsertAsync(userNofitication, cancellationToken: cancellationToken);
                    }
                }

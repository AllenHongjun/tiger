...
namespace Tiger.Module.Notifications
{
    public class StaticNotificationSaver : IStaticNotificationSaver, ITransientDependency
    {
            ...
            IEnumerable<NotificationDefinitionGroupRecord> notificationGroupRecords)
        {
            ...
            foreach (var notificationGroupRecord in notificationGroupRecords)
            {
                ...
                if (notificationGroupRecordInDatabase == null)
                {
                    ...
                    await NotificationGroupRepository.InsertAsync(notificationGroupRecord);
                    continue;
                }
                ...
                await NotificationGroupRepository.UpdateAsync(notificationGroupRecordInDatabase);
            ...
            if (deletedRecords.Any())
            {
                ...
                {
                    await NotificationGroupRepository.DeleteAsync(record);
                }
            }
            ...
            IEnumerable<NotificationDefinitionRecord> notificationRecords)
        {
            ...
            if (newRecords.Any())
            {
                foreach (var notificationRecord in newRecords)
                {
                    await NotificationRepository.InsertAsync(notificationRecord);
                }
            }

            if (changedRecords.Any())
            {
                foreach(var notificationRecord in changedRecords)
                {
                    await NotificationRepository.UpdateAsync(notificationRecord);
                }
            ...
            if (deletedRecords.Any())
            {
                ...
                {
                    await NotificationRepository.DeleteAsync(item);
                }
        ...
        {
            // TODO: IApplicationInfoAccessor 获取applicationName
            return $"{CacheOptions.KeyPrefix}_App_AbpNotificationUpdateLock";
        }
        ...
        {
            return $"{CacheOptions.KeyPrefix}_App_AbpNotificationsHash";
        }

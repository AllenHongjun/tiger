...
using Tiger.Volo.Abp.SettingUi.Extensions;
...
namespace Tiger.Volo.Abp.SettingUi
{
    ...
    public class SettingUiAppService : ApplicationService, ISettingUiAppService
    {
        ...
        {
            return _fileProvider.GetDirectoryContents(SettingUiConst.SettingPropertiesFileFolder)
                .Select(content => _jsonSerializer.Deserialize<IDictionary<string, IDictionary<string, string>>>(content.ReadAsString()))
        ...
        protected virtual async Task<SettingInfo> CreateSettingInfoAsync(SettingDefinition settingDefinition)
        {
            ...
            var settingInfo = new SettingInfo
            {
                ...
                Value = value,
                Properties = new Dictionary<string, object>(),  // new ExtraPropertyDictionary(),
            };

            // Copy properties from SettingDefinition
            foreach (var property in settingDefinition.Properties)
            {
                settingInfo.Properties[property.Key] = property.Value;
            }

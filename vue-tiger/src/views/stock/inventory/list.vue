<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @sort-change="sortChange">
      ...
      <el-table-column max-width="180px" label="商品名称">
        <template slot-scope="{row}">
          <span>{{ row.title }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column width="80px" align="center" label="图标">
        <template slot-scope="scope">
          <span><img :src="scope.row.image_uri" width="30px"></span>
        </template>
      </el-table-column>

      <el-table-column max-width="180px" label="规格型号">
        <template slot-scope="{row}">
          <span>{{ row.author }}</span>
        </template>
      </el-table-column>

      <el-table-column class-name="status-col" label="单位" width="60">
        <template slot-scope="{row}">
          <span>千克</span>
        </template>
      </el-table-column>

      <el-table-column min-width="60px" label="现存量">
        <template slot-scope="{row}">
          <span>{{ row.forecast }}</span>
        </template>
      </el-table-column>

      <el-table-column max-width="120px" label="助记码">
        <template slot-scope="{row}">
          <span>{{ row.author }}</span>
        </template>
      </el-table-column>

      <el-table-column max-width="180px" label="仓库名称">
        <template slot-scope="{row}">
          <span>{{ row.author }}</span>
        </template>
      </el-table-column>

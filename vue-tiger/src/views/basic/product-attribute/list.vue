<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" fit highlight-current-row style="width: 100%" @sort-change="sortChange">
      ...
      <el-table-column width="140px" label="名称">
      ...
      <el-table-column class-name="status-col" label="是否可选" width="110">
        <template slot-scope="{row}">
      ...
      <el-table-column class-name="status-col" label="录入方式" width="110">
        <template slot-scope="{row}">
      ...
      <el-table-column min-width="100px" label="可选值列表">
        <template slot-scope="{row}">
      ...
      <!-- <el-table-column width="180px" align="center" label="修改时间" sortable>
        <template slot-scope="scope">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="120px">
        ...
        <el-form-item label="检索类型" prop="searchType">
          <el-radio-group v-model="temp.searchType">
        ...
        <el-form-item label="手动新增" prop="handAddStatus">
          <el-radio-group v-model="temp.handAddStatus">
        ...
        <el-form-item label="可选值" prop="inputList">
          <el-input v-model="temp.inputList" placeholder="可选值列表，以英文逗号隔开" />
        </el-form-item>
...
import { getProductAttributes, createProductAttribute, updateProductAttribute, deleteProductAttribute } from '@/api/basic/productAttribute'
import { getProductAttributeTypes } from '@/api/basic/productAttributeType'
...
export default {
  ...
  created() {
    this.getList()
    this.getProductAttributeTypeList()
  },
  methods: {
    getList() {
      this.listLoading = true
      getProductAttributes(this.listQuery).then(response => {
        this.list = response.items
        this.total = response.totalCount
        this.listLoading = false
      })
    },
    getProductAttributeTypeList() {
      var listQuery = {
        page: 1,
        limit: 999
      }
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          createProductAttribute(this.temp).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          updateProductAttribute(tempData).then(() => {

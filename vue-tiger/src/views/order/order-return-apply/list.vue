    ...
    <div v-show="billContainerVisibilty" class="bill-container">
      ...
      <el-table ref="billTable" v-loading="listLoading" :data="list" fit highlight-current-row style="width: 100%" @sort-change="sortChange" @current-change="handleCurrentChange">
        ...
        <el-table-column class-name="status-col" label="类型" width="110">
          <template slot-scope="{row}">
            <el-tag :type="row.orderReturnType | statusFilter">
              {{ row.orderReturnType | orderReturnTypeFilter }}
            </el-tag>
          </template>
        </el-table-column>
    ...
    <div v-show="!billContainerVisibilty" class="bill-detail-container">
      ...
      <el-form ref="detailForm" :rules="rules" :model="temp" label-width="120px">
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="类型" prop="orderReturnType">
              <el-select v-model="temp.orderReturnType" class="filter-item" placeholder="请选择">
                <el-option v-for="item in orderReturnTypeOptions" :key="item.key" :label="item.display_name" :value="item.key" />
              </el-select>
            </el-form-item>
          </el-col>
        ...
        <el-row>
          ...
          <el-col :span="6">
            <el-form-item label="总体积" prop="orderReturnType">
        ...
        <el-divider />
        <el-table v-loading="listLoading" :data="temp.orderReturnDetails" fit highlight-current-row style="width: 100%" :show-summary="true" @sort-change="sortChange">
          <el-table-column align="center" type="selection" width="55" />
...
<script>
import { getOrderReturnHeaders, getOrderReturnHeaderById, createOrderReturnHeader, updateOrderReturnHeader, deleteOrderReturnHeader } from '@/api/order/order-return-header'
...
const orderReturnTypeOptions = [
  { key: 0, display_name: '其他入库' },
  { key: 1, display_name: '采购入库' },
  { key: 2, display_name: '退货入库' },
  { key: 3, display_name: '盘盈入库' }
]
const orderReturnTypeKeyValue = orderReturnTypeOptions.reduce((acc, cur) => {
  acc[cur.key] = cur.display_name
  return acc
}, {})
...
export default {
  ...
  filters: {
    ...
    orderReturnTypeFilter(type) {
      return orderReturnTypeKeyValue[type]
    }
  },
  data() {
    return {
  ...
  methods: {
    getList() {
      ...
      getOrderReturnHeaders(this.listQuery).then(response => {
    ...
    createData() {
      this.$refs['detailForm'].validate((valid) => {
        if (valid) {
          ...
          createOrderReturnHeader(this.temp).then((res) => {
            // 添加成功后列表数据操作
    ...
    getDetail(id) {
      // if (this.temp.orderReturnDetails !== null) {
      //   return
      // }
      getOrderReturnHeaderById(id).then((res) => {
        console.log('getOrderReturnHeaderById', res)
        this.temp = Object.assign({}, this.currentRow) // copy obj

        const items = res.orderReturnDetails
        this.temp.orderReturnDetails = items.map(v => {
          this.$set(v, 'edit', false) // https://vuejs.org/v2/guide/reactivity.html
          // v.originalTitle = v.title //  will be used when user click the cancel botton
          return v
        })
        // this.temp.orderReturnDetails = res.orderReturnDetails // copy obj
    ...
    updateData() {
      this.$refs['detailForm'].validate((valid) => {
        if (valid) {
          ...
          updateOrderReturnHeader(tempData).then(() => {
    ...
    handleDelete() {
      this.$confirm('此操作将永久删除数据, 是否继续?', '提示', {
        ...
        deleteOrderReturnHeader(this.currentRow.id)
          .then((response) => {
    ...
    handleCreateDetail(row) {
      // this.temp.orderReturnDetails
      const index = this.temp.orderReturnDetails.indexOf(row)
      // 拼接函数(索引位置, 要删除元素的数量, 元素)
      // this.temp.orderReturnDetails.unshift(this.temp.orderReturnDetails[0])
      console.log('this.temp.orderReturnDetails[0]', this.temp.orderReturnDetails[0])
      console.log('this.temp.orderReturnDetails', this.temp.orderReturnDetails)
      this.resetTempDetail()
      this.temp.orderReturnDetails.splice(index, 0, this.tempDetail)
    },
    // 删除明细
    handleDeleteDetail(row) {
      const index = this.temp.orderReturnDetails.indexOf(row)
      this.temp.orderReturnDetails.splice(index, 1)
    },
    //
    ...
    handleExport() {
      ...
      import('@/vendor/Export2Excel').then(excel => {
        ...
        const data = this.formatJson(filterVal, this.temp.orderReturnDetails)
        excel.export_json_to_excel({
          header: tHeader,
          data,
    ...
    handleSuccess({ results, header }) {
      ...
      console.log(results)
      this.temp.orderReturnDetails = results

...
export function useNotifications() {
  ...
  function onNotifyReceived(notificationInfo, notifer) {
    ...
    if (notificationInfo.type === NotificationType.ServiceCallback) {
      emitter.emit(
        NotifyEventEnum.NOTIFICATIONS_SERVICE_CALLBACK,
        notificationInfo
      )
    } else {
  ...
  function _notification(notifier, severity) {
  ...
  function readNotifer(notifier) {
    signalR
      .invoke('change-state', notifier.id, NotificationReadState.Read)
      .then(() => {

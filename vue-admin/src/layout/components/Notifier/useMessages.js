...
export function useMessages() {
  ...
  onMounted(() => {
    ...
    signalR.on('recall-chat-message', message =>
      emitter.emit(ChatEventEnum.USER_MESSAGE_RECALL, message)
    )
    signalR.on('on-user-onlined', (_, userId) =>
      emitter.emit(ChatEventEnum.USER_ONLINE, userId)
    )
    signalR.on('on-user-offlined', (_, userId) =>
  ...
  function refreshLastMessages(maxResultCount = 10) {
    getLastMessages({
      ...
      messageRef.value.list = res.items.map(message =>
  ...
  function onMessageReceived(message) {
    ...
    if (
      ...
      (message.extraProperties.L === true ||
        message.extraProperties.L === 'true')
    ) {
      message.content = t(
        message.extraProperties.content.ResourceName +
          '.' +
          message.extraProperties.content.Name,
        message.extraProperties.content.Values
      )
    }

...
<el-dialog :title="$t('AbpFeatureManagement[\'Features\']')" :visible.sync="dialogFormVisible">
    <el-form ref="dataForm" :rules="rules" :model="temp" label-position="right" label-width="120px">
        <el-form-item v-for="feature in features" :key="feature.name" :label="feature.displayName">
            <el-checkbox v-if="feature.valueType.name === 'ToggleStringValueType'" v-model="temp[feature.name]" />
            <el-input v-else-if="feature.valueType.name === 'FreeTextStringValueType'" v-model="temp[feature.name]" />
        </el-form-item>

<template>
  <div class="app-container">
    ...
    <el-table v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%" @sort-change="sortChange">
      ...
      <el-table-column min-width="120" label="优惠券名称">
        <template slot-scope="{row}">
          <span>{{ row.title }}</span>
        </template>
      </el-table-column>

      <el-table-column width="120" label="领取人">
        <template slot-scope="{row}">
      ...
      <el-table-column width="80px" align="center" label="最低消费额">
        <template slot-scope="scope">
          <span>{{ scope.row.forecast }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column class-name="status-col" label="是否可用" width="110">
        <template slot-scope="{row}">
          <el-switch
            v-model="show"
            active-color="#13ce66"
      ...
      <el-table-column class-name="status-col" label="获取方式" width="110">
        <template slot-scope="{row}">

<template>
  <div class="app-container">
    <el-dialog :title=" dialogStatus == 'create'? $t('AppPlatform[\'School:AddNew\']'): $t('AbpUi[\'Edit\']')" :visible.sync="dialogFormVisible">
...
export default {
  ...
  methods: {
    ...
    handleUpdate(row) {
      ...
      this.dialogFormVisible = true
      getSchool(row.id).then(response => {
        this.temp = response
      })
    ...
    updateData() {
      this.$refs['dataForm'].validate(valid => {
        if (valid) {
          updateSchool(this.temp.id, this.temp).then(() => {
            this.$emit('handleFilter', false)
            this.dialogFormVisible = false
            this.$notify({
              title: this.$i18n.t("TigerUi['Success']"),
